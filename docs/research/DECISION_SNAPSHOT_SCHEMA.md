...
`LIFECYCLE_TRANSITION` 不是 Decision Class。Lifecycle Transition 使用第 17 節的獨立 Transition Entity。
...
### 18.6 Multi-transition Decisions
...
- Lineage retirement transition completeness
...
18. Multi-transition Decision must use a Transition Group.
...
- [ ] Lineage Retirement 為每個 active version 建立獨立 Transition。
