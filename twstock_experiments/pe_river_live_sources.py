...
def build_yahoo_chart_url(symbol: str) -> str:
    query = urlencode({"interval": "1m", "range": "1d", "includePrePost": "false", "events": "div,splits"})
    return f"{YAHOO_CHART_ENDPOINT.format(symbol=symbol)}?{query}"


def parse_yahoo_quote_payload(payload: dict[str, object], *, expected_symbol: str, source_url: str = "") -> DelayedQuote:
    ...
    if symbol != expected_symbol:
        raise LiveExperimentError(f"Yahoo symbol mismatch: expected {expected_symbol}, got {symbol or 'blank'}")
    ...
    return DelayedQuote(
        ...
        previous_close_twd=optional_positive_decimal(meta.get("previousClose", meta.get("chartPreviousClose"))),
...
def fetch_yahoo_delayed_quote(symbol: str, *, transport: HttpTransport | None = None, timeout: float = 10, retries: int = 2) -> DelayedQuote:
    ...
    return parse_yahoo_quote_payload(payload, expected_symbol=symbol, source_url=response.url)


def _record_from_csv_row(row: dict[str, str]) -> MarketDataRecord:
    try:
        return MarketDataRecord(
            source=row["source"].strip(), source_tier=SourceTier(row["source_tier"].strip()),
            source_symbol=row["source_symbol"].strip(), canonical_symbol=row["canonical_symbol"].strip(),
            market=row["market"].strip(), trade_date=row["trade_date"].strip(),
            traded_share_volume=int(row["traded_share_volume"]), official_traded_value_twd=int(row["official_traded_value_twd"]),
            open_price=float(row["open_price"]), high_price=float(row["high_price"]), low_price=float(row["low_price"]),
            close_price=float(row["close_price"]), transaction_count=int(row["transaction_count"]),
            retrieved_at=row.get("retrieved_at", "").strip(), source_reference=row.get("source_reference", "").strip(),
...
def load_normalized_market_csv(path: Path, *, expected_symbol: str) -> tuple[MarketDataRecord, ...]:
    if not path.is_file():
    ...
    for record in records:
        if record.canonical_symbol != expected_symbol:
            raise LiveExperimentError(f"Market CSV symbol mismatch: {record.canonical_symbol} != {expected_symbol}")
...
def official_close_fallback(records: Sequence[MarketDataRecord]) -> DelayedQuote:
    ...
    return DelayedQuote(
        ...
        previous_close_twd=optional_positive_decimal(records[-2].close_price) if len(records) >= 2 else None,
        source="TWSE_STOCK_DAY", source_tier="PRIMARY",
        data_status="OFFICIAL_COMPLETED_SESSION_CLOSE_FALLBACK", source_url=latest.source_reference,
    )


def resolve_quote(symbol: str, official_records: Sequence[MarketDataRecord], *, transport: HttpTransport | None = None) -> tuple[DelayedQuote, str | None]:
    try:
    ...
    for row_number, row in enumerate(_rows(path), start=2):
        try:
            item = FundamentalObservation(
                period=row["period"].strip(), period_end=_iso(row["period_end"], "period_end"),
                publication_date=_iso(row["publication_date"], "publication_date"),
                research_available_date=_iso(row["research_available_date"], "research_available_date"),
                gross_margin_pct=positive_decimal(row["gross_margin_pct"], field="gross_margin_pct"),
                operating_margin_pct=positive_decimal(row["operating_margin_pct"], field="operating_margin_pct"),
                diluted_eps_twd=positive_decimal(row["diluted_eps_twd"], field="diluted_eps_twd"),
                data_status=row.get("data_status", "UNKNOWN").strip() or "UNKNOWN",
                source_note=row.get("source_note", "").strip(), source_url=row.get("source_url", "").strip(),
            )
        except KeyError as exc:
            raise ExperimentInputError(f"Row {row_number}: missing {exc.args[0]}") from exc
        if item.research_available_date < item.publication_date:
            raise ExperimentInputError(f"Row {row_number}: research_available_date precedes publication_date")
...
def load_monthly_revenue(path: Path) -> list[MonthlyRevenueObservation]:
    ...
    for row_number, row in enumerate(_rows(path), start=2):
        ...
        try:
            ...
            raise ExperimentInputError(f"Row {row_number}: month must be YYYY-MM") from exc
        if month in seen:
            raise ExperimentInputError(f"Row {row_number}: duplicate month {month}")
        seen.add(month)
        observations.append(MonthlyRevenueObservation(
            month=month, revenue_million_twd=positive_decimal(row.get("revenue_million_twd"), field="revenue_million_twd"),
            data_status=row.get("data_status", "UNKNOWN").strip() or "UNKNOWN",
            source_note=row.get("source_note", "").strip(), source_url=row.get("source_url", "").strip(),
        ))
...
def fetch_history(symbol: str, start: date, end: date, *, csv_path: Path | None = None) -> tuple[MarketDataRecord, ...]:
    if csv_path is not None:
        return load_normalized_market_csv(csv_path, expected_symbol=symbol)
    try:
        records = fetch_twse_daily(source_symbol_from_input(symbol), start.isoformat(), end.isoformat())
    except (SourceUnavailableError, DataValidationError, MalformedSourceError) as exc:
        raise LiveExperimentError(f"TWSE history fetch failed for {symbol}: {exc}") from exc
    if not records:
